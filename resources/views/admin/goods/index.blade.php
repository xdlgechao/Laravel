      ...
      <table class="mws-datatable-fn mws-table dataTable" id="DataTables_Table_1" aria-describedby="DataTables_Table_1_info">
        ...
        @foreach($goods as $k=>$v)
          <tr class="@if($k%2==0) even @else odd @endif">
            ...
            <td class=" ">@foreach($v->image as $key => $value)  
            		<img src="{{$value->path}}" width="30" alt="">  
            		@endforeach</td>
            <td class=" "><input cid="{{$v->id}}" class="ibutton" type="checkbox" @if($v->status) checked="checked" @endif></td>
            <td class=" "><a class="btn btn-info" href="/admin/goods/edit?id={{$v->id}}/">修改</a><a class="btn btn-danger" href="/admin/goods/delete?id={{$v->id}}">删除</a></td>
           </tr>
        @endforeach
        </tbody>
      </table>
