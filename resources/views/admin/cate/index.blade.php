      ...
      <table class="mws-datatable-fn mws-table dataTable" id="DataTables_Table_1" aria-describedby="DataTables_Table_1_info">
        <thead>
          <tr role="row">
            ...
            <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending" style="width: 207px;">分类名称</th>
            <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" aria-label="Platform(s): activate to sort column ascending" style="width: 195px;">父级分类</th>
            <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" aria-label="Engine version: activate to sort column ascending" style="width: 134px;">路径</th>
        ...
        @foreach($cates as $k=>$v)
          <tr class="@if($k%2==0) even @else odd @endif">
            ...
            <td class=" ">{{$v->name}}</td>
            <td class=" ">{{$v->names or '顶级分类'}}</td>
            <td class=" ">{{$v->path}}</td>
            <td class=" ">
                <input cid="{{$v->id}}" class="ibutton" type="checkbox" @if($v->status) checked="checked" @endif>
            </td>
            <td class=" "><a class="btn btn-info" href="/admin/cate/edit?id={{$v->id}}">修改</a><a class="btn btn-danger" href="/admin/cate/delete?id={{$v->id}}">删除</a></td>
           </tr>
        @endforeach
        </tbody>
      </table>
